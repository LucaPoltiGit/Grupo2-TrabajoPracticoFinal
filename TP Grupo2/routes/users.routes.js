...
router.put("/agregarPuntajeGanador/:uid", async (req, res) => {
    try {
        const {uid} = req.params
...
console.log("modificando puntos al usuario")
        const puntajeFinal = await userController.modifyPoints(user,points)
...
router.put("/agregarPuntajeEmpate/:uid", async (req, res) => {
    try {
        const {uid} = req.params
